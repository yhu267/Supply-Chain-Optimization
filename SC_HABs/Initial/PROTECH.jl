...
for d in day
    ...
    for l in layer
        #if l <= lm
        #    Pdem[l] = 1.2*sum((exp(rcorθI[a,ll])-1)*X[a,d-1,ll]*(rcorθI[a,ll]>=0) for a in algae for ll in layer)/lm; # P demand for algae to finish growth
        #    Ndem[l] = 8.3*sum((exp(rcorθI[a,ll])-1)*X[a,d-1,ll]*(rcorθI[a,ll]>=0)*(feature[a]!="Yes") for a in algae for ll in layer)/lm; # N demand for algae to finish growth
        #else
...
open("eqm_composition.csv","w") do al
for l in layer
    for a in algae
        print(al, X[a,120,l], ",");
    end
