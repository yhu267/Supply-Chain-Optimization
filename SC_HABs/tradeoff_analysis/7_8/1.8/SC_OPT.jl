...
@variable(m, Cop[TIME] >= 0);     # total oprational cost
@variable(m, Ctrans[TIME] >= 0);  # total transportation cost
@variable(m, Revenue[TIME]);      # total revenue
@variable(m, Csup[TIME]); # total supply cost
@constraint(m, Cinv == 1/40*sum(y[n,t]*tech_inv[t] for n in NODES_C for t in TECHS));
@constraint(m, [tt in TIME], Cop[tt] == sum(g[n,tech_ref[t],t,tt]*tech_op[t] for n in NODES_C for t in TECHS));
@constraint(m, [tt in TIME], Ctrans[tt] == sum(f[i,j,pr,tt]*D[i,j]*prod_trans[pr] for i in NODES for j in NODES for pr in PRODS));
@constraint(m, [tt in TIME], Revenue[tt] == sum(d[n,pr,tt]*sum(dem_price[dem]*(dem_node[dem] == n)*(dem_prod[dem] == pr) for dem in DEMS) for n in NODES for pr in PRODS)); 
@constraint(m, [tt in TIME], Csup[tt] ==    sum(s[n,pr,tt]*sum(sup_price[sup]*(sup_node[sup] == n)*(sup_prod[sup] == pr) for sup in SUPS) for n in NODES for pr in PRODS));
...
@variable(m, sumferP[TIME] >= 0);
@variable(m, sumferN[TIME] >= 0);
...
@constraint(m, [tt in TIME], sumferP[tt] == sum(ferP[n,tt] for n in NODES));
@constraint(m, [tt in TIME], sumferN[tt] == sum(ferN[n,tt] for n in NODES));

# objective function (weights can be flexible)
λ1 = 1;
λ2 = 1.799*33.07*1000; # apoorva $/tonne
λ3 = 1.799*4.54*1000;  # traci equivalent
@objective(m, Min, λ1*(Cinv + sum(Cop[tt] for tt in TIME) + sum(Ctrans[tt] for tt in TIME) + sum(Csup[tt] for tt in TIME) - sum(Revenue[tt] for tt in TIME) + 1.34*1000*sum(sumferP[tt] for tt in TIME) + 0.86*1000*sum(sumferN[tt] for tt in TIME)) + λ2*sumTP + λ3*sumTN);
