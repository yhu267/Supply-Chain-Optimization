...
println("Transportation Cost (\$): ", getvalue(sum(Ctrans[tt] for tt in TIME)))
println("Operating Cost (\$)): ", getvalue(sum(Cop[tt] for tt in TIME)))
println("Total Revenue (\$): ", getvalue(sum(Revenue[tt] for tt in TIME)))
println("Total Supply Cost (\$): ", getvalue(sum(Csup[tt] for tt in TIME)))
...
println("P from fertilizers (t): ", getvalue(sum(sumferP[tt] for tt in TIME)));
println("N from fertilizers (t): ", getvalue(sum(sumferN[tt] for tt in TIME)));
...
open("results_summary.csv","w") do rp
    ...
    println(rp, "Transportation Cost (\$): ",",", getvalue(sum(Ctrans[tt] for tt in TIME)))
    println(rp, "Operating Cost (\$)): ",",", getvalue(sum(Cop[tt] for tt in TIME)))
    println(rp, "Total Revenue (\$): ",",", getvalue(sum(Revenue[tt] for tt in TIME)))
    println(rp, "Total Supply Cost (\$): ",",", getvalue(sum(Csup[tt] for tt in TIME)))
    ...
    println(rp, "P from fertilizers (t): ",",", getvalue(sum(sumferP[tt] for tt in TIME)));
    println(rp, "N from fertilizers (t): ",",", getvalue(sum(sumferN[tt] for tt in TIME)));
...
open("economic_results.csv","w") do ep
	println(ep, "TIME",",","Operational cost (\$)",",","Transportation cost (\$)",",","Revenue (\$)",",","Supply Cost (\$)",",","P fertilizer (tonne)",",","N fertilizer (tonne)")
	for tt in TIME
		println(ep, tt, getvalue(Cop[tt]), ",", getvalue(Ctrans[tt]), ",", getvalue(Revenue[tt]), ",", getvalue(Csup[tt]),",", getvalue(sumferP[tt]), ",", getvalue(sumferN[tt]));
	end
end
...
for pp in PRODS[1:9]
    for t in TIME
    open("flow_results_"*"$(pp)"*"_"*"$(t)"*".csv","w") do ff
	print(ff,",")
	for j in NODES
	    print(ff,j,",")
	end
