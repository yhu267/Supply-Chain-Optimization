...
node_long = Dict(zip(NODES, node_matrix[:,3])); # node longitude
node_lat  = Dict(zip(NODES, node_matrix[:,4])); # node latitude
...
@variable(m, t>=0);
@constraint(m, t >= sum(Pn)-Pn["n1371"]-Pn["n1372"]-1724406.97912*0.000454);
@constraint(m, swf == demrevn - supcost - opcost - transcost - λ*t);
@objective(m, Max, swf)
